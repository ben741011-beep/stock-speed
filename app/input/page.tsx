      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-5rem)] max-w-6xl flex-col">
        <div className="flex flex-1 items-center justify-center py-10"><section className="w-full max-w-xl rounded-3xl border border-white/10 bg-slate-900/70 p-6 shadow-2xl shadow-black/20 backdrop-blur sm:p-8">
          ...
          <form className="mt-8 space-y-5" onSubmit={submit}>
            <label className="block"><span className="mb-2 block text-sm font-semibold text-slate-200">00631L 投資金額</span><div className="input-frame flex items-center rounded-2xl border border-slate-700 bg-slate-950 px-4 focus-within:border-teal-400 focus-within:ring-4 focus-within:ring-teal-400/10"><span className="text-slate-500">NT$</span><input required className="w-full bg-transparent px-3 py-4 text-lg font-semibold outline-none" type="number" min="0" step="1" value={investment} onChange={(event) => setInvestment(event.target.value)} /></div></label>
            <label className="block"><span className="mb-2 block text-sm font-semibold text-slate-200">現金</span><div className="input-frame flex items-center rounded-2xl border border-slate-700 bg-slate-950 px-4 focus-within:border-teal-400 focus-within:ring-4 focus-within:ring-teal-400/10"><span className="text-slate-500">NT$</span><input required className="w-full bg-transparent px-3 py-4 text-lg font-semibold outline-none" type="number" min="0" step="1" value={cash} onChange={(event) => setCash(event.target.value)} /></div></label>
